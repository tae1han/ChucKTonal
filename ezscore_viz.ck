...
class Part extends GGen
{
    ...
    fun void init(EZscore input_melody)
    {
        ...
        mel.totalDuration => totalDuration;
        mel.n_voices => n_voices;
        (notesCount * 2) - pitches[0].size() - pitches[-1].size()  => lineCount; // NEED TO EDIT TO ALLOW CHORDS
        init_spheres();
        init_lines();
        init_sound();
        init_followers();
    ...
    {
        GSphere temp[notesCount] @=> spheres;
        for(auto x : spheres)
        {
        ...
        // x dimension scaled by total rhythmic duration
        max_width / totalDuration => dx;
        0 => float xPos;
        // y dimension scaled to pitch range
        mel.getLowestNote() => lowP;
        mel.getHighestNote() => hiP;
        ...
        for(int i; i < pitches.size(); i++)
        {
            ...
            for(int j; j < curr_notes.size(); j++)
            {
                ...
                if (yPos <= 0) 0 => yPos;
                spheres[sphereIndex].pos(@(xPos, yPos, 0.0));
                chordPositions << @(xPos, yPos, 0.0);
                sphereIndex++;
            }
    ...
    fun void init_lines()
    {
        ...
        for(int i; i < melSize - 1; i++) // each X position along part
        {
            pitches[i].size() => int thisChordSize;
            pitches[(i + 1)%notesCount].size() => int nextChordSize;
            // connect each sphere at this (vertical) position to the bottom sphere of the next position
            for (int p; p < thisChordSize; p++)
            {
                vec3 endpoints[2];
                spheres[(sphereIndex + p)].pos() => endpoints[0];
                spheres[(sphereIndex + thisChordSize)].pos() => endpoints[1];
                endpoints => lines[lineIndex].geo().positions;
                //(lineIndex + 1) % lineCount => lineIndex;
                lineIndex++;
            }
            ...
            for (1 => int q; q < nextChordSize; q++)
            {
                ...
                spheres[(sphereIndex + thisChordSize + q)].pos() => endpoints[1];
                endpoints => lines[lineIndex].geo().positions;
    ...
    {
        Step temp[notesCount] @=> step;
        ADSR temp2[notesCount] @=> height_followers;
    ...
    fun void update()
    {
        ...
        //part_parent.rotateY(-.2 * dt);

        for(int i; i < melSize - 1; i++) // each position along X direction
        {
            pitches[i] @=> int curr_notes[];
            pitches[i].size() => int thisChordSize;
            pitches[(i + 1)].size() => int nextChordSize;
            ...
            for (int j; j < curr_notes.size(); j++)
            {
                ...
                //(sphereIndex + 1) % notesCount => sphereIndex;
                sphereIndex++;
            }
            ...
            {
                vec3 endpoints[2];
                spheres[(lowestSphere + p) % notesCount].pos() => endpoints[0];
                spheres[(lowestSphere + thisChordSize) % notesCount].pos() => endpoints[1];
                endpoints => lines[lineIndex].geo().positions;
                //(lineIndex + 1) % _nLines => lineIndex;
                lineIndex++;
            }
            ...
            for (1 => int q; q < nextChordSize; q++)
            {
                ...
                spheres[(lowestSphere + thisChordSize + q) % notesCount].pos() => endpoints[1];
                endpoints => lines[lineIndex].geo().positions;
                //(lineIndex + 1) % _nLines => lineIndex;
                lineIndex++;
            }
        ...
        // update last sphere position
        pitches[-1] @=> int curr_notes[];
        pitches[-1].size() => int thisChordSize;
        for (int j; j < curr_notes.size(); j++)
        {
